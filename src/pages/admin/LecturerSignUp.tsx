...
const LecturerSignUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const staffId = `${faculty.slice(0, 3).toUpperCase()}/${department.slice(0, 3).toUpperCase()}/${year}${month}${day}${random}`;
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="w-full max-w-2xl p-8 space-y-6 bg-white rounded-lg shadow-md">
        <h2 className="text-2xl font-bold text-center">Create Lecturer Account</h2>
        <form className="space-y-6" onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <input type="text" name="title" placeholder="Title" onChange={handleChange} className="input-style" />
